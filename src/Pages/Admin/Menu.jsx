...
const Menu = () => {
  ...
  const { items, loading, error, successMessage } = useSelector((state) => state.menu);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await dispatch(updateMenu({ id: editingId, data: formData })).unwrap();
      } else {
  ...
  const columns = [
    ...
    { key: 'name', label: 'Menu Name' },
    {
      ...
      label: 'Position',
      render: (pos) => {
        const positions = { header: 'Header', footer: 'Footer', sidebar: 'Sidebar' };
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-primary mb-2">Menu Name *</label>
            <PrimaryInput
              type="text"
              name="name"
          ...
          <div>
            <label className="block text-sm font-medium text-primary mb-2">Position</label>
            <select
